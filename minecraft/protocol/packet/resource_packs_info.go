...
// ResourcePacksInfo is sent by the server to inform the client on what resource pack the server has. It
// sends a list of the resource pack it has and basic information on them like the version and description.
type ResourcePacksInfo struct {
	...
	// HasAddons specifies if any of the resource pack contain addons in them. If set to true, only clients
	// that support addons will be able to download them.
	HasAddons bool
	// HasScripts specifies if any of the resource pack contain scripts in them. If set to true, only clients
	...
	// The order of these texture pack is not relevant in this packet. It is however important in the
